...
(function ($, exports) {
    $(function () {
        ...
        var Tag={
            type:0,
            refresh: function(fn){
                $.post("",{type:this.type},function(data){fn(data)});
            }
        };
        $("#aside-content1").delegate("p", "click", function () {
            ...
            $("#tag-manage>li:eq("+(_type-1)+")").show().siblings().hide();
        });
...
(function ($, exports) {
    $("#edit_tag").click(function(){
        $(this).hide();
        $("#exit_edit_tag").show();
        $("#my_tags .tagCol").find("a").each(function(){
            $(this).siblings("b").append("<span style='color: #EB7638'>X</span>").parent().wrapInner("<span class='edit_tag_item'></span>");
        });
    });

    $("#exit_edit_tag").click(function(){
        $(this).hide();
        $("#edit_tag").show();
        $("#my_tags .tagCol").find("a").each(function(){
            $(this).siblings("b").find("span").remove().end().parent().parent().html($(this).parent(".edit_tag_item").html());
        });
    });
