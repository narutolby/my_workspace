...
<div id="article">
    ...
    <div class="bResrcC">
        ...
        <ul class="discuss-list" id="comments">
        </ul>
...
<script type="text/x-jquery-tmpl" id="comment_tmpl">
              <li>
                ...
                <div class="disuss-body">
                    <div class="disuss-head">
                        ...
                        <div class="disuss-time"> <span>评论时间：{{= pubDate}}</span></div>
                    </div>
...
<script type="text/javascript">
    $L.wait(function(){
        ...
        $("#comment_tmpl").template("comment_tmpl");
        var comments = $("#comments");
        function fetchComments(data){
            if(data){
                var html = $.tmpl("comment_tmpl",data.comments);
                comments.find("li").remove().end().append(html);
                return ;
            }
            $.get("r_comm/get_comments.json",{resource_id:'${resource.id}'},function(data){
                var html = $.tmpl("comment_tmpl",data.comments);
                comments.find("li").remove().end().append(html);
            });
        }
        fetchComments();
        var _reply_input = $("#reply_input");
        $("#btn_reply").bind("click",function(){
            _reply_input.attr("disabled",true);
            $.get("r_comm/save_comment.json",{comment:_reply_input.val(),resource_id:'${resource.id}'},function(data){
                fetchComments(data);
                _reply_input.attr("disabled",false).val("");
            });
        });
    });
</script>
